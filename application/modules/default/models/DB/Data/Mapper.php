...
class Model_DB_Data_Mapper extends Model_Abstract_DBMapper {
    ...
    protected function getWhereClauseByFilter ( Model_Data_Filter $filter ) {
        ...
        if ( $filter->getId() ) {
            ...
            $value = $filter->get();
            if ( !empty( $value ) ) {
                $where .= $and . $adapter->quoteInto( Model_DB_Data_Table::FIELDS_ID_USER_FK . " = ?", $value );
            }
            ...
            if ( !empty( $value ) ) {
                $where .= $and . $adapter->quoteInto( Model_DB_Data_Table::FIELDS_ID_YEAR_FK . " = ?", $value );
            }
            $value = $filter->getSemester();
            if ( !empty( $value ) ) {
                $where .= $and . $adapter->quoteInto( Model_DB_Data_Table::FIELDS_SEMESTER . " = ?", $value );
            }
        }
        return $where;
    }
